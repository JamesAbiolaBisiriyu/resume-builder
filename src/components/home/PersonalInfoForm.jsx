      ...
      {field.map((item) => {
        ...
        return(
          <div key={item.key} className="space-y-1 mt-5">
            ...
            <input type={item.type} value={data[item.key] || ""}
            onChange={(e)=>handleChange(item.key, e.target.value)} className="mt-1 w-full px-3 py-2 border 
            border-gray-300 rounded-lg focus:ring focus:ring-blue-500 focus:border-blue-500
            outline-none transition-colors text-sm" placeholder={`Enter your ${item.label.toLowerCase()}`} required={item.required} />
          </div>
        )
      })}
    </div>
